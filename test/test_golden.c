/*
 * Hand-rolled scanner tailored exactly to the golden.json schema emitted by
 * python/export.py. Not a general JSON parser: it anchors on the fixed key
 * order (name, inputs, ts, hidden_states, outputs) per sequence and reads a
 * known number of floats after each key, ignoring JSON punctuation entirely.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
typedef struct {
    char name[32];
    float inputs[MAX_TIMESTEPS][CFC_INPUT_DIM];
    float ts[MAX_TIMESTEPS];
    float hidden_states[MAX_TIMESTEPS][CFC_HIDDEN_DIM];
    float outputs[MAX_TIMESTEPS][CFC_HIDDEN_DIM];
} golden_seq_t;
...
static const char *find_key(const char *p, const char *key) {
    ...
    if (!loc) {
        fprintf(stderr, "key not found in golden.json: %s\n", key);
        exit(2);
    }
...
static const char *parse_sequence(const char *p, golden_seq_t *seq) {
    ...
    p = find_key(p, "inputs");
    p = read_floats(p, &seq->inputs[0][0], MAX_TIMESTEPS * CFC_INPUT_DIM);
    p = find_key(p, "ts");
    p = read_floats(p, seq->ts, MAX_TIMESTEPS);
    p = find_key(p, "hidden_states");
    p = read_floats(p, &seq->hidden_states[0][0], MAX_TIMESTEPS * CFC_HIDDEN_DIM);
    p = find_key(p, "outputs");
    p = read_floats(p, &seq->outputs[0][0], MAX_TIMESTEPS * CFC_HIDDEN_DIM);
    return p;
}

static void check_dim(const char *name, int got, int expect) {
    if (got != expect) {
        fprintf(stderr, "golden.json %s mismatch: got %d, expected %d\n", name, got, expect);
        exit(2);
    }
}

int main(int argc, char **argv) {
    const char *path = (argc > 1) ? argv[1] : "../weights/generated/golden.json";
    ...
    check_dim("input_dim", input_dim, CFC_INPUT_DIM);
    check_dim("hidden_dim", hidden_dim, CFC_HIDDEN_DIM);
    ...
    for (int s = 0; s < MAX_SEQUENCES; s++) {
        ...
        for (int t = 0; t < MAX_TIMESTEPS; t++) {
            ...
            cfc_step_backbone(&state, seq->inputs[t], seq->ts[t], output);
